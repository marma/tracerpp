...
//	  vector rotate(double a,double b) { double sina=sin(a),cosa=cos(a),sinb=sin(b),cosb=cos(b); return tracer::vector(x*cosa*cosb - y*sina + z*cosa*sinb, x*sina*cosb + y*cosa + z*sina*sinb, -x*sinb + z*cosb); }
	  ...
	  friend inline vector operator * (double d, const vector& v) {
	    return tracer::vector(d*v.x, d*v.y, d*v.z);
	  }
