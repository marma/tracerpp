...
namespace tracer {
  ...
  color preview(const ray& r, scene& world) {
    ...
    if (ivect.size() == 0) {
      return 0.25*tracer::color(0.6, 0.65, 0.8);
    } else {
      intersection i = *ivect.begin();
      double d = -i.world_n * r.v;

      d = fabs(d);

      if (d>0) {
	// diffuse
	return d*tracer::white;
      } else {
	return tracer::black;
      }
    }
  ...
  color ambient(const vector& p, const vector& n, scene& world, double angle, int nsamples, double t) {
